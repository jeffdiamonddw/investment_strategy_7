...
class RegimeNavigator0D(RegimeNavigator1D):
    """
    ...
    def run_simulation(self, w_mom_vals, w_qual_vals, max_voo, period_key, sim_id, session=None, holdings=None):
        
        
        period = self.periods[period_key]
        dates = self.df_macro.index[(self.df_macro.index >= period['val_start_date']) & (self.df_macro.index <= period['end_date'])]
        dates = self.df_macro.index[(self.df_macro.index >= period['val_start_date']) & (self.df_macro.index <= period['end_date'])]
        ...
        df = pd.concat([df_mom, df_qual], axis = 1)
        df = df.div(df.sum(axis = 1), axis = 0)
        df_weights = pd.concat([df] * len(dates), ignore_index=True).set_index(pd.Index(dates))
    ...
    def evaluate(self, x):
        ...
        for key in self.periods:
            df_period = self.run_simulation(w_mom, w_qual, max_voo, key, sim_id, holdings = self.holdings)
...
if __name__ == "__main__":
    ...
    periods = {
        'train': {'train_start_date': pd.to_datetime('2020-01-01'), 'val_start_date': pd.to_datetime('2025-07-01'), 'end_date': pd.to_datetime('2026-09-01')},
    }
    ...
    regime_navigator = RegimeNavigator0D(**problem_args)
